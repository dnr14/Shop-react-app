...
const useBoards = LOCAL_STORAGE_ID => {
  ...
  const [pageNationState, setPageNagetion] = useState(getPageNationInitialData(LOCAL_STORAGE_ID));
  ...
  const removeRowsCheckedClick = useCallback(
    id => e => {
      setRemoveRowIds(prevIds =>
        e.target.checked ? [...new Set([...prevIds, id])] : prevIds.filter(prevId => prevId !== id),
      );
    },
    [],
  );
  ...
  const removeRows = useCallback(() => {
    ...
    const newArray = expendArray.filter(el => (removeRowIds.includes(el.id) ? false : true));
